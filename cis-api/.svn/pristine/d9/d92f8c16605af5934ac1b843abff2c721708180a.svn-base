...
import uk.ac.reigate.dto.ilp.MassILPPraiseLetterDto
...
class MassILPLetterService {
     ...
     */
    @Transactional
    public void createMassPraiseEmail(MassILPPraiseLetterDto massILPPraiseLetterDto) {
        //load the statement bank object from the database and for each student in the studentList create an ILP Interview
        StatementBank statementBank
        if(massILPPraiseLetterDto.statementId != null) {
            statementBank = statementBankService.findById(massILPPraiseLetterDto.statementId)
        }
        if(massILPPraiseLetterDto.studentList != null) {
            massILPPraiseLetterDto.studentList.each { it ->
                Student student = studentService.findById(it)
                if (student != null) {
                    Staff staffRequesting = staffService.findById(massILPPraiseLetterDto.staffId)
                    LetterType letterType = letterTypeService.findById(massILPPraiseLetterDto.letterTypeId)
                    
                    // Load related data
                    //                    Student currentStudent = students.get(i)
                    CourseGroup courseGroup = courseGroupService.findById(massILPPraiseLetterDto.courseGroupId)
                    StudentYear studentYear = studentYearService.findByStudentAndYear(student, courseGroup.year)
                    
                    // Create ILP Entry
                    ILPInterview ilpInterview = new ILPInterview()
                    ilpInterview.student = student
                    if (massILPPraiseLetterDto.courseGroupId != null && courseGroup != null) {
                        ilpInterview.courseGroup = courseGroup
                    }
                    ilpInterview.staff = staffRequesting
                    ilpInterview.discussion = replaceForMassLetter(statementBank.discussion, null, ilpInterview.staff, null, null, ilpInterview.student, courseGroup)
                    ilpInterview.interviewDate = massILPPraiseLetterDto.interviewDate
                    ilpInterview.type = ilpInterviewTypeService.findById(massILPPraiseLetterDto.interviewTypeId)
                    ilpInterviewService.save(ilpInterview)
                    // Create ILP Letter
                    Letter letter = new Letter()
                    letter.student = student
                    letter.requestedDate = massILPPraiseLetterDto.interviewDate
                    letter.letterType = letterType
                    letter.interview = ilpInterview
                    letter.processingFlag = 1
                    letter.authorisedDate = letter.letterType.requireAuthorisation == false ? new Date() : null;
                    ...
                    if (massILPPraiseLetterDto.letterTypeId != null) {
                        ilpInterview.letterSent = true
                    }
                    letterService.save(letter)
                }
