...
public class StatementBankApi {
    ...
    @RequestMapping(value = "/mass-praise-email", produces = ["application/json"], method = RequestMethod.GET)
    public ResponseEntity<List<StatementBankDto>> getMassPraiseLetters() throws NotFoundException {
        LOGGER.info("** StatementBanksApi - statementBanksGet");
        List<StatementBank> statementBanks = statementBankService.findUseForMassPraiseLetters();
        return new ResponseEntity<List<StatementBankDto>>(StatementBankDto.mapFromList(statementBanks), HttpStatus.OK);
    }
